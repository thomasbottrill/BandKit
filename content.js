(() => {
  ...
  function ensurePageStyles() {
    if (!document.querySelector("#bandcamp-hub-page-style")) {
      ...
      pageStyle.textContent += `:is(header,#user-nav,ul[role="menubar"].menu-items) :is(a[href*="/cart"],a[href*="bandcamp.com/cart"],[aria-label*="cart" i],[title*="cart" i],[data-testid*="cart" i],.cart-link,#cart-link,#cart-control){display:none!important}:is(header,#user-nav,ul[role="menubar"].menu-items) li:has(> :is(a[href*="/cart"],a[href*="bandcamp.com/cart"],[aria-label*="cart" i],[title*="cart" i],[data-testid*="cart" i],.cart-link,#cart-link,#cart-control)){display:none!important}`;
  ...
  async function init() {
    ...
    function syncNativeHeaderCart(nav) {
      ...
      const roots = [
        ...
        ...document.querySelectorAll("header .menu-items, ul[role='menubar'].menu-items"),
        document.querySelector("menu-bar")?.shadowRoot
      ].filter(Boolean);
      const nativeCarts = [...new Set(roots.flatMap((headerRoot) => [...headerRoot.querySelectorAll(nativeHeaderCartSelector)]))]
