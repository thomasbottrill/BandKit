(() => {
  // src/content/core.js
  var asset = (name) => chrome.runtime.getURL("assets/" + name);
  ...
  function formatCartPrice(value, currency = "USD") {
    ...
    try {
      const formatted = new Intl.NumberFormat(void 0, { style: "currency", currency: code, currencyDisplay: "narrowSymbol" }).format(amount);
  ...
  function safeBandcampReleaseUrl(value) {
    ...
    try {
      ...
      const publicHost = hostname && hostname !== "localhost" && !hostname.endsWith(".localhost") && !hostname.endsWith(".local") && !hostname.includes(":") && !privateIpv4;
      return url.protocol === "https:" && !url.username && !url.password && (!url.port || url.port === "443") && publicHost && /^\/(?:album|track)\/[^/]+/.test(url.pathname) ? url.href : "";
    } catch {
      return "";
    }
  }
  function portableBandcampUrl(value, releaseOnly = false) {
  ...
  function parseColor(value) {
    ...
    return { r: Number(match[1]), g: Number(match[2]), b: Number(match[3]), a: match[4] === void 0 ? 1 : Number(match[4]) };
  }
  ...
  function readableColor(foreground, backgrounds, minimum) {
    ...
    const target = Math.min(...backgrounds.map((background) => contrast(white, background))) >= Math.min(...backgrounds.map((background) => contrast(black, background))) ? white : black;
  ...
  function accessibleControlPalette(backgrounds, preferredBackground) {
    ...
    const foregroundFor = (background2) => contrast(background2, white) >= contrast(background2, black) ? white : black;
    const passes = (background2) => surfaces.every((surface) => contrast(background2, surface) >= 3) && contrast(background2, foregroundFor(background2)) >= 4.5;
    let background = preferredBackground;
    if (!background || !passes(background)) {
      background = [black, white].sort((first, second) => Math.min(...surfaces.map((surface) => contrast(second, surface))) - Math.min(...surfaces.map((surface) => contrast(first, surface))))[0];
    }
  ...
  var MAX_PLAYLIST_ITEMS = 500;
  var DEFAULT_DATA_PARENT = "documents";
  var DEFAULT_DATA_FOLDER = "Bandkit";
  var FEEDBACK_FORM_URL = "https://fir-fruitadens-b76.notion.site/1ddbcd8a2d994a1c9e6f40906a34d5cd?pvs=105";
  var FEEDBACK_LIST_URL = "https://fir-fruitadens-b76.notion.site/9eb94149121d422e8068ffad2b67007b?v=92708ba1dd9a4ee1b86e449e4219650f";
  var SUPPORT_PAYMENT_URL = "https://buy.stripe.com/cNi00jgyeeifgIIdPU6Ri00";
  var MUSIC_BAR_WIDTHS = ["tight", "default", "wide", "full", "custom"];
  var THEME_COLOR_KEYS = ["accent", "surface", "card", "background", "pageSurface", "navbar", "text", "secondaryText"];
  var BUILT_IN_THEMES = [
  ...
  function createPlaylistModel({
    ...
    portableBandcampUrl: portableBandcampUrl2,
    resolveImage: resolveImage2,
    resolvedTrackPageUrl: resolvedTrackPageUrl2,
    safeBandcampUrl: safeBandcampUrl2
  }) {
    function canonicalPlaylistPageUrl(track) {
      let pageUrl = resolvedTrackPageUrl2(track);
      try {
    ...
    function playlistTracksMatch(left, right) {
      ...
      return Boolean(leftPage && leftPage === rightPage && leftTitle && leftTitle === rightTitle && (!leftArtist || !rightArtist || leftArtist === "bandcamp" || rightArtist === "bandcamp" || leftArtist === rightArtist));
    }
    ...
    function normalizePlaylistItem(track, index = 0) {
      ...
      const pageUrl = resolvedTrackPageUrl2(track);
      ...
      return {
        ...
        art: resolveImage2(track.art).slice(0, 4e3),
        pageUrl,
        artistUrl: safeBandcampUrl2(track.artistUrl),
    ...
    function portablePlaylistItem(item) {
      ...
      return {
        ...portable,
        pageUrl: portableBandcampUrl2(portable.pageUrl, true),
        artistUrl: portableBandcampUrl2(portable.artistUrl)
      };
    }
    function normalizeSavedPlaylists(savedPlaylists) {
      return (Array.isArray(savedPlaylists) ? savedPlaylists : []).map((snapshot, index) => ({
        ...
        savedAt: snapshot?.savedAt || (/* @__PURE__ */ new Date()).toISOString(),
        sourcePage: portableBandcampUrl2(snapshot?.sourcePage),
    ...
    function mergeHydratedPlaylist(currentItems, hydratedItems) {
      ...
      return normalizePlaylist(normalizePlaylist(currentItems).map((current) => {
        const refreshed = byItemId.get(current.playlistItemId) || hydrated.find((item) => playlistTracksMatch(item, current));
        return refreshed ? {
          ...current,
          ...refreshed,
  ...
  function portableCartRestore(restore) {
    ...
    return ["a", "t", "b", "p"].includes(portable.item_type) && Number.isFinite(Number(portable.item_id)) ? portable : null;
  }
  function portableCartItem(item, { includeRestore = true } = {}) {
    if (!item || typeof item !== "object") return null;
    return {
      ...
      art: typeof item.art === "string" ? item.art.slice(0, 4e3) : "",
  ...
  function parseCartBackup(text) {
    ...
    const items = payload.items.slice(0, 100).map((item, index) => {
      ...
      return {
        ...
        price: Math.max(0, Number(item.price) || 0),
        currency: /^[A-Z]{3}$/.test(item.currency || "") ? item.currency : "USD",
        art: /^https?:/i.test(item.art || "") ? String(item.art).slice(0, 4e3) : "",
        url,
    ...
    return {
      items,
      savedAt: Number.isNaN(exportedAt.getTime()) ? (/* @__PURE__ */ new Date()).toISOString() : exportedAt.toISOString(),
  ...
  (async () => {
    ...
    const modernPlaylistQueueCache = /* @__PURE__ */ new Map();
    ...
    const observedAudio = /* @__PURE__ */ new WeakSet();
    ...
    function openFeedFromBandcampHome() {
      if (state.openHomeToFeed !== true || location.hostname !== "bandcamp.com" || location.pathname !== "/" || location.hash.startsWith("#bandkit-")) return false;
      const feedUrl = canonicalBandcampFeedUrl(state.feedUrl) || discoverBandcampFeedUrl();
      if (!feedUrl) return false;
      location.replace(feedUrl);
      return true;
    }
    const cartArtistCache = /* @__PURE__ */ new Map();
    const cartArtistPending = /* @__PURE__ */ new Set();
    const cartArtistAttempted = /* @__PURE__ */ new Set();
    ...
    let pageTrackAnalysis = /* @__PURE__ */ new Map();
    ...
    const pageTrackPriceCache = /* @__PURE__ */ new Map();
    const pageTrackPricePending = /* @__PURE__ */ new Set();
    ...
    document.addEventListener("bandkit:media-state", (event) => {
      ...
      if (hubReady && (!previous || previous.src !== bridgedMedia.src || previous.paused !== bridgedMedia.paused || previous.ended !== bridgedMedia.ended || previous.duration !== bridgedMedia.duration)) {
        window.clearTimeout(playerEventScanTimer);
        playerEventScanTimer = window.setTimeout(scanLivePlayer, 0);
      }
      if (seamless.enabled && !bridgedMedia.paused && !feedSwitchPendingDisable && !pendingFeedTrackId && !discoverSwitchPendingDisable && Date.now() >= collectionNativeFallbackUntil) {
  ...
  `;
    ...
    function syncPlayerPageSpace() {
      ...
      const pageFlowParent = document.querySelector("#DiscoverApp main.app") || pageFooter?.parentElement || document.querySelector("#propOpenWrapper, .full-page-app-wrapper") || document.body;
    ...
    function refreshScrubWaveform(force = false, observedWidth = 0) {
      const signature = `${live.title || ""}\0${live.artist || ""}\0${live.pageUrl || ""}`;
    ...
    function syncScrubVisual(value = Number(scrubSlider.value) || 0) {
      const progress = Math.max(0, Math.min(1, value / 1e3));
    ...
    function syncMusicBarWidth() {
      ...
      const customWidth = Math.round(Math.max(480, Math.min(2e3, Number(state.musicBarCustomWidth) || 900)));
    ...
    for (const tab of tabs) {
      ...
      const shortcutCounter = tab.id === "cart" ? '<span class="hub-header-shortcut-count hub-cart-shortcut-count" aria-hidden="true">0</span>' : tab.id === "playlist" ? '<span class="hub-header-shortcut-count hub-playlist-shortcut-count" aria-hidden="true">0</span>' : '<span class="hub-header-shortcut-dot" aria-hidden="true"></span>';
    ...
    function individualTrackPageUrl(track) {
      ...
      for (const row of document.querySelectorAll(".track_row_view, .track-list-item, [data-trackid], [data-track-id]")) {
        ...
        const link = [...row.querySelectorAll("a[href]")].find((anchor) => {
          try {
            return new URL(anchor.href, location.href).pathname.includes("/track/");
          } catch {
            return false;
          }
        });
    ...
    function nativeTrackActionControl(action) {
      const selectors = action === "wishlist" ? ["#wishlist-msg button", "#wishlist-msg a", ".wishlist-button", "button[data-action*='wish' i]", "button[aria-label*='wishlist' i]", "a[aria-label*='wishlist' i]"] : ["#buyItem", ".buyItem", ".buy-link", "button[data-action*='cart' i]", "button[aria-label*='cart' i]"];
      for (const selector of selectors) {
        for (const match of document.querySelectorAll(selector)) {
          ...
          const control = match.matches("button, a, [role='button']") ? match : match.querySelector("button, a, [role='button']") || match.closest("button, a, [role='button']");
    ...
    function activateNativeTrackAction(action) {
      ...
      if (action === "wishlist") {
        ...
        const alreadyWishlisted = control.getAttribute("aria-pressed") === "true" || control.classList.contains("wishlisted") || /in wishlist|wishlisted|remove from wishlist/.test(status);
    ...
    function nativeCheckoutControl() {
      ...
      if (!match) return null;
      return match.matches("button, a, [role='button']") ? match : match.querySelector("button, a, [role='button']") || match.closest("button, a, [role='button']");
    }
    ...
    function runPendingTrackAction() {
      const action = location.hash === "#bandkit-wishlist" ? "wishlist" : location.hash === "#bandkit-cart" ? "cart" : location.hash === "#bandkit-checkout" ? "checkout" : "";
      if (!action) return;
      let attempts = 0;
      ...
      const tryAction = () => {
        ...
        if (activated) {
          ...
          showToast(action === "checkout" ? "Opening Bandcamp checkout\u2026" : action === "wishlist" ? "Opened Bandcamp's wishlist action" : "Opened Bandcamp's purchase action");
          return;
        }
    ...
    async function openTrackAction(track, action) {
      ...
      if (action === "wishlist") markWishlistTrack(track, true);
      showToast(action === "wishlist" ? `Opened \u201C${track.title}\u201D on Bandcamp for your wishlist.` : `Opened \u201C${track.title}\u201D on Bandcamp for purchase.`);
    }
    ...
    function artistUrlFromPageUrl(value) {
      ...
      try {
        ...
        return url.hostname === "bandcamp.com" ? pageUrl : url.hostname.endsWith(".bandcamp.com") ? `${url.origin}/` : pageUrl;
      } catch {
        return pageUrl;
      }
    }
    ...
    function reportStorageWriteError(error) {
      const now = Date.now();
      if (now - storageErrorToastAt < 5e3) return;
      storageErrorToastAt = now;
      const message = String(error?.message || error || "");
      showToast(/quota|bytes|space/i.test(message) ? "Bandkit storage is full. Delete old playlists or cart backups." : "Bandkit could not save this change.");
    }
...
`);
    ...
    async function savePortableDataHome() {
      if (!("showDirectoryPicker" in window)) {
        const backup2 = portableDataBackup();
        const blobUrl = URL.createObjectURL(new Blob([`${JSON.stringify(backup2, null, 2)}
`], { type: "application/json" }));
        const link = document.createElement("a");
        link.href = blobUrl;
        link.download = `bandkit-data-${backup2.exportedAt.slice(0, 10)}.json`;
        link.click();
      ...
      const rootDirectory = dataDirectoryHandle.name.toLowerCase() === DEFAULT_DATA_FOLDER.toLowerCase() ? dataDirectoryHandle : await dataDirectoryHandle.getDirectoryHandle(DEFAULT_DATA_FOLDER, { create: true });
      ...
      state.dataFolderName = dataDirectoryHandle.name.toLowerCase() === DEFAULT_DATA_FOLDER.toLowerCase() ? `Documents/${DEFAULT_DATA_FOLDER}` : `${dataDirectoryHandle.name}/${DEFAULT_DATA_FOLDER}`;
      saveState();
    ...
    function syncPageTypography() {
      ...
      const fontFamily = pageFont && pageFont !== "initial" ? pageFont : '"Helvetica Neue", Helvetica, Arial, sans-serif';
    ...
    function addTracksToPlaylist(tracks, { quiet = false } = {}) {
      ...
      for (const source of incoming) {
        ...
        source.addedAt ||= (/* @__PURE__ */ new Date()).toISOString();
      ...
      const capacitySuffix = atCapacity ? ` \xB7 ${MAX_PLAYLIST_ITEMS}-track limit reached` : "";
      if (!quiet) showToast(added ? `Added ${added} track${added === 1 ? "" : "s"} to Now Playing${capacitySuffix}` : atCapacity ? `Now Playing can hold up to ${MAX_PLAYLIST_ITEMS} tracks` : "Already in Now Playing");
      return added;
    }
    ...
    function addTracksToSavedPlaylist(tracks, snapshotId) {
      ...
      for (const item of incoming) {
        ...
        item.addedAt = (/* @__PURE__ */ new Date()).toISOString();
      ...
      const capacitySuffix = atCapacity ? ` \xB7 ${MAX_PLAYLIST_ITEMS}-track limit reached` : "";
      showToast(added ? `Added ${added} track${added === 1 ? "" : "s"} to \u201C${snapshot.name}\u201D${capacitySuffix}` : atCapacity ? `\u201C${snapshot.name}\u201D can hold up to ${MAX_PLAYLIST_ITEMS} tracks` : `Already in \u201C${snapshot.name}\u201D`);
      return added;
    }
    ...
    async function refreshIncompletePlaylistMetadata() {
      ...
      const repair = (item) => {
        ...
        const artist = (!currentArtist || currentArtist.toLowerCase() === "bandcamp") && replacementArtist.toLowerCase() !== "bandcamp" ? replacementArtist : currentArtist || replacementArtist || "Bandcamp";
    ...
    async function prepareExternalNowPlaying(track, sourceQueue = [], { trustProvidedStreams = false, replaceQueue = false } = {}) {
      ...
      if (!preserveQueue) {
        ...
        state.playlistMode = "browse";
        state.playlist = pageQueue.some((item) => playlistTracksMatch(item, incoming)) ? pageQueue : normalizePlaylist([incoming, ...pageQueue.slice(0, MAX_PLAYLIST_ITEMS - 1)]);
      }
      if (!state.playlist.length) {
        ...
        if (trustProvidedStreams && isReusableStreamUrl(incoming.url)) {
          const queue3 = initialQueue.filter((item) => isReusableStreamUrl(item.url));
          const index3 = queue3.findIndex((item) => Boolean(matchingQueueTrack([item], incoming)));
          if (index3 >= 0) {
            state.playlist = initialQueue;
            saveState();
            if (state.activeTab === "nowPlaying") render();
            injectPlaylistButtons();
            return { queue: queue3, index: index3, error: "", request: prepareRequest, usedFreshStreams: true };
          }
        }
        const hydrated2 = await hydratePlaylist(initialQueue);
        ...
        const mergedItems = mergeHydratedPlaylist(state.playlist.length ? state.playlist : initialQueue, hydrated2.items);
        const queue2 = mergedItems.filter((item) => isReusableStreamUrl(item.url));
        const index2 = queue2.findIndex((item) => Boolean(matchingQueueTrack([item], incoming)));
        if (index2 >= 0) {
        ...
        return {
          queue: queue2,
          index: index2,
          error: index2 >= 0 ? "" : hydrated2.error || "This Bandcamp track is not currently streamable.",
      ...
      const promoted = {
        ...
        addedAt: existing?.addedAt || incoming.addedAt || (/* @__PURE__ */ new Date()).toISOString()
      };
      const nextItems = [
        promoted,
        ...currentItems.filter((_, index2) => index2 !== existingIndex)
      ];
      ...
      if (trustProvidedStreams && isReusableStreamUrl(promoted.url)) {
        const queue2 = state.playlist.filter((item) => isReusableStreamUrl(item.url));
        const index2 = queue2.findIndex((item) => item.playlistItemId === promoted.playlistItemId);
        if (index2 >= 0) {
          return { queue: queue2, index: index2, error: "", request: prepareRequest, usedFreshStreams: true };
        }
      }
    ...
    async function playPlaylistAt(requestedIndex = 0, { forceRefresh = false } = {}) {
      ...
      const nextItems = state.playlistMode === "manual" ? [selected, ...state.playlist.filter((_, index2) => index2 !== selectedIndex)] : state.playlist.slice(selectedIndex);
      const queueChanged = nextItems.length !== state.playlist.length || nextItems.some((item, index2) => item.playlistItemId !== state.playlist[index2]?.playlistItemId);
      state.playlist = normalizePlaylist(nextItems);
      ...
      try {
        ...
        saveState();
        render();
        showToast(`Playing ${queue.length} playlist track${queue.length === 1 ? "" : "s"}${hydrated.failed ? ` \xB7 ${hydrated.failed} unavailable` : ""}`);
        return true;
      } finally {
    ...
    async function navigatePlayerQueue(direction) {
      ...
      const playlistIndex = seamless.track ? state.playlist.findIndex((item) => Boolean(matchingQueueTrack([item], seamless.track))) : -1;
      if (playlistIndex >= 0 && state.playlist.length > seamlessQueue.length) {
        let targetIndex2 = playlistIndex + step;
        while (targetIndex2 >= 0 && targetIndex2 < state.playlist.length) {
          const targetItemId = state.playlist[targetIndex2]?.playlistItemId;
          if (await playPlaylistAt(targetIndex2)) return true;
    ...
    function restoreSavedPlaylist(snapshot, mode = "replace", { syncPlayback = true } = {}) {
      ...
      showToast(mode === "replace" ? `Loaded \u201C${snapshot.name}\u201D into Now Playing` : addedCount ? `Added ${addedCount} track${addedCount === 1 ? "" : "s"} from \u201C${snapshot.name}\u201D to Now Playing` : `No new tracks from \u201C${snapshot.name}\u201D were added`);
    }
    function shareFileName(value, fallback) {
      const name = String(value || "").trim().toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-+|-+$/g, "").slice(0, 72);
    ...
    function sharedCollectionDocument({ title, eyebrow, summary, rows, embeddedPayload, embeddedId }) {
      ...
      const embeddedData = embeddedPayload && embeddedId ? `<script type="application/json" id="${escapeHtml(embeddedId)}">${embeddedPayload}<\/script>` : "";
      return `<!doctype html>
<html lang="en">
<head>
  <meta charset="utf-8">
...
<body>
  ...
  ${embeddedData}
    ...
    function createPlaylistDocument(items = state.playlist, playlistName = "Bandkit playlist") {
      ...
      const rows = playlistItems.map((item) => {
        ...
        const chips = [
          ...
          [item.key?.camelot, item.key?.shortName].filter(Boolean).join(" \xB7 ")
      ...
      return {
        ...
        html: sharedCollectionDocument({
          ...
          summary: `${playlistItems.length} track${playlistItems.length === 1 ? "" : "s"} \xB7 Shared ${exportedAt.toLocaleString()}`,
          rows,
          embeddedPayload,
    ...
    async function shareHtmlDocument(documentData, label) {
      ...
      downloadHtmlDocument(documentData, `Sharing is unavailable here \xB7 downloaded ${label} instead`);
    }
    ...
    function parsePlaylistBackup(text) {
      ...
      if (source.startsWith("{")) {
        ...
        if (embedded) {
          ...
          const items2 = legacyRows.map((row, index) => {
          ...
          if (!items2.length) throw new Error("This HTML file does not contain recognizable Bandkit playlist tracks.");
          payload = {
            format: "bandkit-playlist",
            version: 1,
      ...
      return {
        ...
        savedAt: Number.isNaN(new Date(payload.exportedAt).getTime()) ? (/* @__PURE__ */ new Date()).toISOString() : new Date(payload.exportedAt).toISOString(),
        sourcePage: portableBandcampUrl(payload.sourcePage),
        items
      };
    }
    ...
    function portableAppearanceTheme(theme = selectedAppearanceTheme()) {
      ...
      return {
        ...
        card: theme.card || hexString(luminance(surfaceColor) < 0.34 ? mixColor(surfaceColor, { r: 255, g: 255, b: 255, a: 1 }, 0.07) : mixColor(surfaceColor, { r: 255, g: 255, b: 255, a: 1 }, 0.4)),
...
`], { type: "application/json" }));
    ...
    function accessibleAppearanceTheme(theme = selectedAppearanceTheme()) {
      const white = { r: 255, g: 255, b: 255, a: 1 };
      const black = { r: 17, g: 24, b: 39, a: 1 };
      const panel2 = hexColor(theme.surface, white);
      const panelIsDark = luminance(panel2) < 0.34;
      const card = hexColor(theme.card, panelIsDark ? mixColor(panel2, white, 0.07) : mixColor(panel2, white, 0.4));
      const background = hexColor(theme.background, luminance(panel2) < 0.34 ? mixColor(panel2, black, 0.38) : mixColor(panel2, white, 0.28));
      const pageSurface = hexColor(theme.pageSurface, panel2);
      ...
      const panelTextResult = readableColor(preferredText, [panel2], 7);
      ...
      const panelMutedResult = readableColor(preferredSecondaryText, [panel2], 4.5);
      ...
      const panelAccentResult = readableColor(preferredAccent, [panel2], 4.5);
      ...
      const panelBorder = readableColor(mixColor(panel2, panelTextResult.color, luminance(panel2) < 0.34 ? 0.24 : 0.16), [panel2], 3).color;
      ...
      return {
        ...
        navbar,
        text,
        panelText: panelTextResult.color,
        cardText: cardTextResult.color,
        panelMuted: panelMutedResult.color,
        cardMuted: cardMutedResult.color,
        backgroundText: backgroundTextResult.color,
        backgroundMuted: backgroundMutedResult.color,
        backgroundAccent: backgroundAccentResult.color,
        backgroundBorder,
        backgroundOnAccent,
        navbarText: navbarTextResult.color,
        accent,
        panelAccent,
        cardAccent: cardAccentResult.color,
        preferredScrubAccent,
        muted,
        border,
        panelBorder,
        cardBorder,
        ...
        textContrast: Math.min(contrast(panelTextResult.color, panel2), contrast(cardTextResult.color, card), contrast(text, pageSurface), contrast(backgroundTextResult.color, background), contrast(navbarTextResult.color, navbar)),
        accentContrast: Math.min(contrast(panelAccent, panel2), contrast(cardAccentResult.color, card), contrast(accent, pageSurface))
      };
    }
    ...
    function applyShadowHeaderTheme() {
      ...
      const propertyNames = [
        ...
        "--elevated-background-color-1",
        "--elevated-background-color-2",
        "--elevated-background-color-3",
        "--elevated-background-color-4",
        "--default-foreground-color",
        "--page-text-color",
        "--page-text-color-secondary",
        "--bc-link-color",
        "--menubar-search-input-background-color",
        "--border-color",
        "--border-color-strong",
        "--border-color-subtle",
        "--bandcamp-blue",
    ...
    function isClassicReleasePage() {
      return Boolean(
        document.body?.classList.contains("tralbum-page") && document.querySelector("#trackInfo") && document.querySelector("#tralbumArt") && document.querySelector(".trackView")
      );
    }
    function modernBandcampPageType() {
      ...
      const isFanCollection = Boolean(document.querySelector("#fan-container, #collection-grid")) && !document.body?.classList.contains("feed") && !/\/feed$/.test(pathname);
    ...
    function markModernTrackAvailability(trackTable) {
      ...
      for (const [index, row] of rows.entries()) {
        ...
        const nativelyUnavailable = !control || control.hidden || control.getAttribute("aria-disabled") === "true" || nativeStyle?.display === "none" || nativeStyle?.visibility === "hidden";
    ...
    function modernArtistForeground(preferred, background, minimum = 4.5) {
      ...
      const distance = Math.sqrt(
        (preferred.r - background.r) ** 2 + (preferred.g - background.g) ** 2 + (preferred.b - background.b) ** 2
      );
    ...
    function accessibleModernPagePalette(palette) {
      const white = { r: 255, g: 255, b: 255, a: 1 };
      const black = { r: 17, g: 24, b: 39, a: 1 };
      const opaque = (color, background2) => {
        const source = color || background2;
        const alpha = Math.max(0, Math.min(1, Number(source?.a ?? 1)));
        return alpha >= 0.999 ? { ...source, a: 1 } : mixColor(background2, source, alpha);
      };
      ...
      const foreground = (preferred, on, minimum = 7) => readableColor(opaque(preferred, on), [on], minimum).color;
      ...
      const role = (on) => {
        ...
        const secondary = palette.preserveArtistColors ? modernArtistForeground(opaque(preferredMuted, on), on, 4.5) : foreground(preferredMuted, on, 4.5);
        const link = palette.preserveArtistColors ? modernArtistForeground(opaque(preferredAccent, on), on, 4.5) : foreground(preferredAccent, on, 4.5);
        return {
          text,
          secondary,
          link,
      ...
      const navbarAccent = palette.preserveArtistColors ? modernArtistForeground(opaque(preferredAccent, navbar), navbar, 3) : foreground(preferredAccent, navbar, 3);
      return {
        ...palette,
        background,
        surface,
    ...
    function clearModernReleasePalette() {
      for (const name of [
        "--bandkit-release-bg",
        "--bandkit-release-surface",
        "--bandkit-release-surface-raised",
        "--bandkit-release-background-ink",
        "--bandkit-release-ink",
        "--bandkit-release-muted",
        "--bandkit-release-line",
        "--bandkit-release-accent",
        "--bandkit-release-accent-soft",
        "--bandkit-release-on-accent",
        "--bandkit-release-raised-ink",
        "--bandkit-release-raised-muted",
        "--bandkit-release-raised-line",
        "--bandkit-release-raised-accent",
        "--bandkit-release-raised-accent-soft",
        "--bandkit-release-raised-on-accent",
        "--bandkit-release-navbar",
        "--bandkit-release-navbar-text",
        "--bandkit-release-navbar-accent",
        "--bandkit-release-navbar-line",
        "--bandkit-release-footer-bg",
        "--bandkit-release-footer-ink",
        "--bandkit-release-footer-muted",
        "--bandkit-release-footer-line",
        "--bandkit-release-footer-accent",
        "--bandkit-release-footer-accent-soft",
    ...
    function syncPageDjTheme() {
      ...
      for (const name of [
        ...
        "--hub-wash",
        "--hub-card-accent",
        "--hub-card-accent-soft",
        "--hub-card-ink",
        "--hub-card-muted",
        "--hub-card-faint",
        "--hub-card-line",
        "--hub-card-on-accent",
        "--hub-card-footer-accent",
        "--hub-card-footer-accent-soft",
        "--hub-card-footer-ink",
        "--hub-card-footer-muted",
        "--hub-card-footer-faint",
        "--hub-card-footer-line",
    ...
    function applyShadowHeaderCartVisibility(hidden) {
      ...
      visibilityStyle.textContent = hidden ? `li.cart,li[data-bandkit-native-cart-wrapper],button[aria-label="Cart"],button[data-bandkit-native-cart]{display:none!important}` : "";
    ...
    function capturePanelLayout() {
      ...
      if (state.layoutMode === "docked") {
        ...
        if (!rect2.width) return;
        state.dockedWidth = Math.round(rect2.width);
    ...
    function createCartDocument(items = state.cart, cartName = "Bandcamp cart", summary = state.cartSummary, { includeRestore = true } = {}) {
      ...
      const rows = cartItems.map((item) => {
        ...
        const artist = item.artist ? artistUrl ? `<a href="${escapeHtml(artistUrl)}">${escapeHtml(item.artist)}</a>` : escapeHtml(item.artist) : "Unknown artist";
        const chips = [
          item.kind,
      ...
      return {
        ...
        html: sharedCollectionDocument({
          ...
          summary: `${cartItems.length} item${cartItems.length === 1 ? "" : "s"} \xB7 ${cartTotalLabel(cartItems, summary)} \xB7 Shared ${exportedAt.toLocaleString()}`,
          rows,
    ...
    function exportActivity() {
      ...
      const rows = state.activity.map((item) => {
        ...
        const timestamp = createdAt && !Number.isNaN(createdAt.getTime()) ? createdAt.toLocaleString() : item.time || "Recorded locally";
        const track = trackUrl ? `<a href="${escapeHtml(trackUrl)}" target="_blank" rel="noopener">${escapeHtml(item.title || "Untitled track")}</a>` : `<strong>${escapeHtml(item.title || "Untitled track")}</strong>`;
        const artist = artistUrl ? `<a href="${escapeHtml(artistUrl)}" target="_blank" rel="noopener">${escapeHtml(item.artist || "Unknown artist")}</a>` : escapeHtml(item.artist || "Unknown artist");
        return `
      <li>
    ...
    async function restoreSavedCart(items) {
      ...
      const summary = result.error ? result.error : `Restored ${result.added || 0}; ${result.alreadyPresent || 0} already present${failed ? `; ${failed} could not be restored` : ""}.`;
      showToast(summary);
    ...
    async function addResolvedItemsToCart(sourceItems) {
      ...
      const result = await new Promise((resolve) => {
        ...
        }, Math.max(15e3, resolved.items.length * 11e3));
    ...
    async function addTracksToSavedCart(tracks, snapshotId) {
      ...
      snapshot.modifiedAt = (/* @__PURE__ */ new Date()).toISOString();
    ...
    function render() {
      ...
      for (const button of headerShortcuts.querySelectorAll(".hub-header-shortcut")) {
        ...
        const count = button.dataset.tab === "cart" ? state.cart.length : button.dataset.tab === "playlist" ? state.savedPlaylists.length : 0;
    ...
    function mountPanelClose() {
      const anchor = state.activeTab === "cart" ? content.querySelector(".hub-saved-cart-detail-toolbar, .hub-cart-view-header") : state.activeTab === "playlist" ? content.querySelector(".hub-saved-cart-detail-toolbar, .hub-section-heading") : state.activeTab === "nowPlaying" ? content.querySelector(".hub-section-heading") : content.querySelector(".hub-section-heading");
      if (!anchor) return;
    ...
    function createDjToolsCard({ includeWaveform = true } = {}) {
      ...
      const refreshDisplayedBpm = () => {
        const tempoAdjusted = Boolean(bpm && Math.abs(state.dj.rate - 1) > 1e-3);
        bpmLabel.textContent = `BPM \xB7 ${state.dj.autoTempo ? "AUTO" : "MANUAL"}${tempoAdjusted ? " \xB7 \u2195" : ""}`;
      ...
      tap.addEventListener("click", () => {
        ...
        let tappedBpm = 6e4 / (intervals.reduce((sum, interval) => sum + interval, 0) / intervals.length);
        ...
        dial.addEventListener("pointerdown", (event) => {
          ...
          const move = (moveEvent) => {
            ...
            if (mode === "radial") {
              ...
              const dragDistance = mode === "vertical" ? vertical : mode === "horizontal" ? horizontal : Math.abs(horizontal) >= Math.abs(vertical) ? horizontal : vertical;
      ...
      eqPanel.append(addKnob({
        label: "Filter",
        value: Math.round(state.dj.filterValue * 100),
        min: -100,
        max: 100,
        format: (value) => value < -1 ? "LP" : value > 1 ? "HP" : "Off",
        onInput: (value) => {
          state.dj.filterValue = value / 100;
          applyDjToAudio();
        }
      }));
      ...
      platterAdjustments.append(
        ...
        addKnob({
          ...
          onInput: (value) => {
            state.dj.vinylSpeedAdjust = value / 100;
          }
        })
      );
      ...
      platter.addEventListener("pointerdown", (event) => {
        ...
        const move = (moveEvent) => {
          if (!dragging2 || moveEvent.pointerId !== pointerId) return;
          moveEvent.preventDefault();
        ...
        const finish = (finishEvent) => {
          if (!dragging2 || finishEvent?.pointerId !== void 0 && finishEvent.pointerId !== pointerId) return;
          dragging2 = false;
          cleanup();
          try {
            platter.releasePointerCapture(pointerId);
          } catch {
          }
          if (!moved) {
      ...
      const rangeLabel = (range) => range === 50 ? "Wide" : `\xB1${range}`;
      ...
      const tempoSlider = addFader({
        ...
        onReset: () => {
          state.dj.rate = 1;
          state.dj.preservePitch = true;
        }
      });
      addFader({
        label: "Gain",
        value: state.dj.gainDb,
        min: -30,
        max: 6,
        step: 1,
        className: "is-gain",
    ...
    function applySeamlessState(nextState) {
      ...
      const editingDjControl = bpmEditing || activeDjControl && djDrawer.contains(activeDjControl) && activeDjControl.matches("input") || activePageDjControl && pageDjSurface?.contains(activePageDjControl) && activePageDjControl.matches("input");
    ...
    function matchingQueueTrack(queue, requested = {}) {
      ...
      if (requestedPageUrl) {
        ...
        const pageMatch = tracks.find((track) => canonicalPlaylistPageUrl(track) === requestedKeyUrl && (!requestedTitle || normalizedTrackTitle(track.title) === requestedTitle));
    ...
    function classicPageTrackForControl(control) {
      ...
      const title = row ? elementText(row, [".track-title", ".title"]) : elementText(control?.closest(".inline_player"), [".title", ".track-title"]);
    ...
    function renderCartViewTabs(metaText) {
      ...
      const tabs2 = createElement("div", "hub-cart-view-tabs");
      tabs2.setAttribute("role", "tablist");
      for (const tab of [
      ...
      header.append(tabs2, createElement("span", "hub-cart-view-meta", metaText));
    ...
    function renderSavedCartList() {
      ...
      for (const snapshot of state.savedCarts) {
        ...
        bodyCopy.append(
          titleRow,
          createElement("div", "hub-saved-cart-date", `${snapshot.autoSaved ? "Auto-saved \xB7 " : ""}${validDate ? savedAt.toLocaleString() : "Saved locally"}`),
          createElement("div", "hub-saved-cart-summary", `${items.length} item${items.length === 1 ? "" : "s"}${items.length ? ` \xB7 ${items.slice(0, 3).map((item) => item.title).join(", ")}${items.length > 3 ? ` +${items.length - 3} more` : ""}` : ""}`)
        );
    ...
    function renderSavedCartDetail(snapshot) {
      ...
      actionRow.append(
        createElement("div", "hub-cart-backup-time", `${items.length} item${items.length === 1 ? "" : "s"} \xB7 ${cartTotalLabel(items, snapshot.summary)}`),
    ...
    function bindPlaylistCardReordering(card, item, index, items, moveItem) {
      ...
      card.addEventListener("drop", (event) => {
        ...
        const destinationIndex = sourceIndex < index ? index - (dropAfter ? 0 : 1) : index + (dropAfter ? 1 : 0);
    ...
    function playlistItemMatchesActiveTrack(item) {
      ...
      return Boolean(
        item.playlistItemId && seamless.track.playlistItemId === item.playlistItemId || matchingQueueTrack([seamless.track], item)
      );
    }
    function syncCurrentPlaylistPlaybackUi() {
      ...
      for (const card of content.querySelectorAll(".hub-playlist-track[data-playlist-item-id]")) {
        ...
        const label = isActuallyLoading ? `Loading ${item?.title || "track"}` : isCurrent ? isPlaying ? `Pause ${item?.title || "track"}` : `Resume ${item?.title || "track"}` : `Play ${item?.title || "track"}`;
        toggle.title = label;
    ...
    function createPlaylistAnalysisMeta(item) {
      ...
      if (key) {
        const keyLabel = [key.camelot, key.shortName].filter(Boolean).join(" \xB7 ") || key.name;
    ...
    function renderPlaylistTrack(item, index) {
      ...
      const isCurrentLoading = isCurrentPlaylistTrack && pendingPlaylistItemId === item.playlistItemId && !playlistItemMatchesActiveTrack(item);
      ...
      mediaToggle.title = isCurrentLoading ? `Loading ${item.title}` : isCurrentPlaylistTrack ? isCurrentPlaying ? `Pause ${item.title}` : `Resume ${item.title}` : `Play ${item.title}`;
      ...
      const details = [item.album, item.duration ? formatDuration(item.duration) : "", item.restoreError].filter(Boolean).join(" \xB7 ");
    ...
    function renderPageQueueFallback() {
      ...
      const queue = [activeTrack, ...live.tracks].filter(Boolean).filter((track, index, items) => items.findIndex((candidate) => Boolean(matchingQueueTrack([candidate], track))) === index).slice(0, 20);
      if (!queue.length) {
    ...
    function renderSavedPlaylistDetail(snapshot) {
      ...
      actionRow.append(
        createElement(
          "div",
          "hub-cart-backup-time",
          `${snapshot.items.length} track${snapshot.items.length === 1 ? "" : "s"} \xB7 ${formatDuration(duration)}`
        ),
    ...
    function renderSavedPlaylists() {
      ...
      for (const snapshot of state.savedPlaylists) {
        ...
        bodyCopy.append(
          ...
          createElement("div", "hub-saved-cart-summary", `${snapshot.items.length} track${snapshot.items.length === 1 ? "" : "s"}${snapshot.items.length ? ` \xB7 ${snapshot.items.slice(0, 3).map((item) => item.title).join(", ")}${snapshot.items.length > 3 ? ` +${snapshot.items.length - 3} more` : ""}` : ""}`)
        );
    ...
    function renderSettings() {
      ...
      const availableThemes = [...BUILT_IN_THEMES, ...state.appearance.savedThemes || [], {
        ...
        const resolvedCard = card || hexString(luminance(surfaceColor) < 0.34 ? mixColor(surfaceColor, { r: 255, g: 255, b: 255, a: 1 }, 0.07) : mixColor(surfaceColor, { r: 255, g: 255, b: 255, a: 1 }, 0.4));
        ...
        for (const [key, label] of [
          ...
          input.addEventListener("input", () => {
            ...
            if (key === "customAccent" && !state.appearance.customScrubAccent) {
              const scrubInput2 = customColours.querySelector('[data-theme-colour="customScrubAccent"]');
              if (scrubInput2) scrubInput2.value = input.value;
            }
            applyAppearance();
            saveState();
          });
      ...
      if (!state.appearance.pageAware) {
        ...
        accessibility.append(
          ...
          createElement("span", "", `Text ${accessibleTheme.textContrast.toFixed(1)}:1 \xB7 controls ${accessibleTheme.accentContrast.toFixed(1)}:1`)
        );
      ...
      dataHomeCopy.append(
        ...
        createElement("span", "hub-data-home-location", state.dataFolderName ? `Location: ${state.dataFolderName}` : `Not set \xB7 default: Documents/${DEFAULT_DATA_FOLDER}`)
      );
    ...
    function renderPlayer() {
      ...
      const trackRenderSignature = hasCurrentTrack ? `${live.title}\0${live.artist}\0${live.art}\0${live.pageUrl}\0${live.artistUrl}` : "empty";
      ...
      if (!scrubbing) {
        scrubSlider.value = String(duration ? Math.round(Math.max(0, Math.min(1, currentTime / duration)) * 1e3) : 0);
        currentTimeLabel.textContent = formatDuration(currentTime);
      }
      syncScrubVisual();
      refreshScrubWaveform();
      scrubSlider.setAttribute("aria-valuetext", `${formatDuration(Number(scrubSlider.value) / 1e3 * duration)} of ${formatDuration(duration)}`);
      scrubControl.classList.toggle("is-empty", !duration);
      durationLabel.textContent = formatDuration(duration);
      const queuedTracks = state.playlist.length ? state.playlist.length : seamless.enabled && Array.isArray(seamless.queue) && seamless.queue.length ? seamless.queue.length : (live.hasPlaybackStarted ? 1 : 0) + (Array.isArray(live.tracks) ? live.tracks.length : 0);
    ...
    function setDjTempo(percent) {
      ...
      state.dj.rate = Math.round((1 + clamped / 100) * 1e3) / 1e3;
      applyDjToAudio();
    }
    function applyDjToAudio(target = getAudio()) {
      ...
      if (Math.abs(target.playbackRate - state.dj.rate) > 1e-3) target.playbackRate = state.dj.rate;
    ...
    function modernPlayerSourceContext(player2, key) {
      if (!key) return null;
      const sourceControl = [...document.querySelectorAll(".play-pause-button[tracklistkey]")].find((control) => !player2.contains(control) && control.getAttribute("tracklistkey") === key);
    ...
    function modernTrackPageUrl(node, sourceContext) {
      ...
      for (const scope of scopes) {
        ...
        const attributeUrl = itemData.item_url || node.getAttribute("data-track-url") || node.getAttribute("trackurl") || node.getAttribute("data-item-url") || node.getAttribute("itemurl");
    ...
    function modernTrackArtistUrl(node, pageUrl) {
      for (const scope of [node, node.closest("li")].filter(Boolean)) {
        for (const link of scope.querySelectorAll?.("a[href]") || []) {
          const identifiesArtist = link.querySelector?.(".artist-name") || /^by\s+/i.test(String(link.textContent || "").trim());
    ...
    async function loadModernPlaylistQueue(modern) {
      ...
      if (!modernPlaylistQueueCache.has(cacheKey)) {
        modernPlaylistQueueCache.set(cacheKey, (async () => {
          ...
          while (nextCursor !== null && nextCursor !== void 0 && tracks.length < Math.min(MAX_PLAYLIST_ITEMS, seed.totalCount)) {
      ...
      const queue = [];
      const sourceToQueue = /* @__PURE__ */ new Map();
    ...
    function getModernPlayerState() {
      const player2 = document.querySelector("section.floating-player.has-track, section.floating-player:has(.track-meta[streamurl])");
      if (!player2) return null;
      const key = player2.querySelector(".play-pause-button:is(.outline, .outline-opaque)[tracklistkey], .play-pause-button[tracklistkey]")?.getAttribute("tracklistkey") || "";
      const sourceContext = modernPlayerSourceContext(player2, key);
      ...
      const sourceAlbum = sourceItemData.item_title || (sourceContext ? elementText(sourceContext, [".collection-item-title", ".release-title", ".title"]) : "") || "";
      const candidates = [...player2.querySelectorAll(".meta-wrapper-wide .track-meta[streamurl], .track-meta[streamurl]")];
      const unique = [...new Map(candidates.map((node) => [node.getAttribute("streamurl"), node])).values()];
      const domQueue = unique.map((node, index2) => {
        const pageUrl = modernTrackPageUrl(node, sourceContext);
        const artist = elementText(node, [".artist-name"]).replace(/^by\s+/i, "") || "Bandcamp";
        return {
          id: node.id || `${node.getAttribute("streamurl")}|${index2}`,
          title: elementText(node, [".title-text", ".track-title", ".title"]) || `Bandcamp track ${index2 + 1}`,
          artist,
      ...
      const seedQueue = playlistSeed?.tracks.map((item, sourceIndex2) => modernPlaylistTrack(item, playlistSeed.title, sourceIndex2)).filter(Boolean) || [];
      ...
      const sourceIndex = currentControl?.hasAttribute("trackindex") ? Number(currentControl.getAttribute("trackindex")) : Number(queue[index]?.sourceIndex ?? index);
      const timeline = player2.querySelector("input[type='range']");
      const isPlaying = Boolean(player2.querySelector(".play-pause-button[aria-label='Pause']"));
      return {
        player: player2,
        queue,
        index,
    ...
    function getDiscoverPlayerState() {
      const player2 = document.querySelector(".discover-player");
      if (!player2) return null;
      const control = player2.querySelector(".play-pause-button");
      const focused = player2.closest(".focused-result") || player2.parentElement;
      const links = [...player2.querySelectorAll(".player-info a[href], a[href]")];
      ...
      const title = elementText(player2, [".player-info .title", ".title"]);
      if (!title) return null;
      ...
      const displayedCurrentTime = parseClock(elementText(player2, [".playback-time.current"]));
      const displayedDuration = parseClock(elementText(player2, [".playback-time.total"]));
    ...
    function getFeedPlayerState(preferredTrackId = "") {
      ...
      const matchingNodes = [...document.querySelectorAll(".collection-item-container[data-trackid]")].filter((node) => node.dataset.trackid === trackId);
    ...
    async function handoffModernPlayer(requestedIndex = null) {
      ...
      const requestedTrackKey = playlistTrackKey(modern.queue.find((track) => Number(track.sourceIndex) === sourceIndex)) || `${modern.key}:${sourceIndex}`;
      ...
      try {
        ...
        if (index === void 0 || !loaded.queue[index]) return false;
    ...
    async function handoffDiscoverPlayer() {
      ...
      try {
        ...
        const handoffCurrentTime = Number.isFinite(latestCurrentTime) ? Math.max(0, latestCurrentTime) : Math.max(0, Number(discover.currentTime) || 0);
        silenceNativePagePlayback();
        const response = await runtimeMessage({
          type: MESSAGES.SEAMLESS_ENABLE,
          queue: prepared.queue,
    ...
    function matchingPagePlaybackTrack(queue, requested = {}) {
      ...
      return tracks.find((track) => normalizedTrackTitle(track.title) === requestedTitle && normalizedTrackTitle(track.artist) === requestedArtist) || null;
    }
    ...
    function clearBandKitPagePlaybackState() {
      for (const player2 of document.querySelectorAll(".inline_player, .track_row_view")) {
        player2.classList.remove("bandcamp-hub-current", "bandcamp-hub-is-playing");
      }
    ...
    function capturePagePlayerUi() {
      ...
      const visualControls = [...document.querySelectorAll(".inline_player .playbutton, .inline_player .play_status, .track_row_view .playbutton, .track_row_view .play_status")].map((element) => ({
        element,
      ...
      const actions = [...new Set(visualControls.map(({ element }) => element.closest("a, button, [role='button']") || element))].map((element) => ({
        element,
    ...
    function syncPagePlayerUi() {
      ...
      for (const player2 of document.querySelectorAll(".inline_player, .track_row_view")) {
        const isInlinePlayer = player2.matches(".inline_player");
        const isCurrent = isInlinePlayer || classicTrackRowMatches(player2, seamless.track, pageData);
        player2.classList.toggle("bandcamp-hub-current", isCurrent);
        player2.classList.toggle("bandcamp-hub-is-playing", isCurrent && seamless.isPlaying);
        const control = player2.querySelector(".playbutton, .play_status, .play_cell a, [aria-label*='Play'], [aria-label*='Pause']");
        ...
        const visualControls = [...player2.querySelectorAll(".playbutton, .play_status")];
    ...
    function populatePageWaveform(control, signature, width = 0) {
      ...
      if (control.dataset.bandkitWaveformSignature === signature && Math.abs(Number(control.dataset.bandkitWaveformWidth) - pixelWidth) < 4) return;
    ...
    function syncPageScrubberContrast(control) {
      ...
      surface ||= luminance(parseColor(getComputedStyle(document.body).color) || { r: 17, g: 24, b: 39, a: 1 }) > 0.5 ? { r: 17, g: 24, b: 39, a: 1 } : { r: 255, g: 255, b: 255, a: 1 };
      ...
      const preferredAccent = sharedPlayerAccent || parseColor(
        styles.getPropertyValue("--hub-scrub-accent") || styles.getPropertyValue("--hub-accent") || styles.getPropertyValue("--bandkit-page-accent") || styles.getPropertyValue("--link-color")
    ...
    function ensurePagePlayerWaveforms() {
      ...
      if (modernTimeline) {
        ...
        const signature = `${modernState?.track?.title || ""}\0${modernState?.track?.artist || ""}\0${modernState?.track?.pageUrl || "modern-player"}`;
      ...
      if (classicControl) {
        ...
        const signature = `${track?.title || elementText(inlinePlayer, [".title", ".track-title"])}\0${track?.artist || ""}\0${track?.pageUrl || location.href}`;
    ...
    function renderPagePlaylistMenu(selection, view = "destinations") {
      ...
      if (view === "playlists") {
        ...
        pagePlaylistMenu.append(option("\uFF0B New playlist", () => {
          closePagePlaylistMenu();
          createSavedPlaylistWithTracks(tracks, isBatch ? firstTrack.album || `${firstTrack.artist || "Bandcamp"} playlist` : `${firstTrack.artist || "Bandcamp"} playlist`);
        }));
        for (const snapshot of state.savedPlaylists) {
        ...
        pagePlaylistMenu.append(
          option(inPlaying ? isBatch ? "\u2713 All in Now Playing" : "\u2713 In Now Playing" : isBatch ? "\uFF0B Add all to Now Playing" : "\uFF0B Add to Now Playing", () => {
            closePagePlaylistMenu();
    ...
    function localTrackPrice(track) {
      ...
      const matchesCurrentTrack = current?.type === "track" && (currentId && currentId === String(track?.id || "") || normalizedTrackTitle(current?.title) === normalizedTrackTitle(track?.title));
    ...
    function resolveTrackPrice(button, track) {
      ...
      void runtimeMessage({ type: MESSAGES.RESOLVE_CART_METADATA, items: [{ url: key }] }).then((response) => {
        ...
        const price = metadata?.itemType === "track" && Number.isFinite(minimumPrice) && minimumPrice >= 0 ? { label: compactPagePrice(minimumPrice, metadata.currency || pageCartCurrency()), minimumPrice } : null;
    ...
    function digitalAlbumPriceLabel() {
      ...
      const priceCopy = digitalOffer?.querySelector(".ft.compound-button.main-button, .ft .main-button, .ft")?.textContent?.replace(/buy\s+digital\s+(?:album|track)/ig, " ").replace(/or\s+more/ig, " ").replace(/\s+/g, " ").trim() || "";
    ...
    function updatePageCartButton(button, track) {
      ...
      const label = price ? `Buy ${itemLabel} for ${price} or more, or open the Bandkit cart` : "Open Bandcamp purchase options and Bandkit cart";
      button.title = label;
      button.setAttribute("aria-label", label);
    }
    function pageAnalysisTrackKey(track) {
      return `${String(track?.id || "")}\0${String(track?.url || "")}`;
    }
    ...
    function renderPageAnalysisLabel(label, result, analyzing = false) {
      ...
      label.setAttribute("aria-label", parts.unavailable ? `Track analysis: BPM unavailable${text.replace(/^— BPM(?: · )?/, "").trim() ? ` \xB7 ${text.replace(/^— BPM(?: · )?/, "")}` : ""}` : `Track analysis: ${text}`);
      if (parts.unavailable) label.title = "BPM could not be detected for this track";
      else if (result.key?.name && state.showTrackKeys !== false) label.title = `${text} \xB7 ${result.key.name}`;
    ...
    function syncPageTrackAnalysisUi() {
      ...
      const inlineText = pageAnalysisLabel(inlineResult) || (inlineTrack && pageTrackAnalysisStatus === "analyzing" ? "Analyzing\u2026" : "");
    ...
    async function analyzePageTracks({ manual = false, force = false } = {}) {
      ...
      if (signature !== pageTrackAnalysisSignature) pageTrackAnalysis = /* @__PURE__ */ new Map();
      ...
      try {
        ...
        const worker = async () => {
          while (cursor < tracks.length) {
            ...
            const result = response?.ok ? response.results?.[0] : { id: track.id, url: track.url, error: response?.error || "Track analysis failed." };
    ...
    function maybeAnalyzePageTracks() {
      ...
      if (signature !== pageTrackAnalysisSignature) {
        pageTrackAnalysisSignature = signature;
        pageTrackAnalysis = /* @__PURE__ */ new Map();
    ...
    function currentInlinePlaylistTrack() {
      ...
      return queue.find((track) => String(track.title || "").replace(/\s+/g, " ").trim().toLowerCase() === normalizedTitle) || queue[0] || null;
    }
    function injectPageDjToolsLink() {
      ensurePageStyles();
      syncPageActionLabelMode();
      const player2 = document.querySelector(".inline_player");
      if (!player2) return;
      let tools = player2.querySelector(":scope > .bandcamp-hub-page-tools");
      if (!tools) {
        tools = document.createElement("div");
        tools.className = "bandcamp-hub-page-tools";
        player2.append(tools);
      }
      ...
      let inlineHost = player2.querySelector(":scope > .bandcamp-hub-page-dj-host");
      if (!inlineHost) {
...
:host{display:block}.hub-page-dj-surface{background:var(--hub-card);border:1px solid var(--hub-line);border-radius:6px;padding:12px;width:100%}.hub-page-dj-surface .hub-dj-card{background:transparent;border:0;box-shadow:none;margin:0;overflow:visible;padding:0}`;
    ...
    function feedTrackFromAction(action) {
      ...
      const title = itemData.featured_track_title || elementText(story, [".fav-track-title", ".featured-track .title", ".track-title", ".collection-item-title"]);
      if (!pageUrl || !title) return null;
      const trackId = String(metadataNode?.dataset.trackid || story.querySelector("[data-trackid]")?.dataset.trackid || itemData.featured_track_id || title);
      const liveFeedTrack = getFeedPlayerState()?.track;
      return {
        id: trackId,
        title,
        artist: itemData.band_name || elementText(story, [".collection-item-artist", ".artist-name", ".band-name"]).replace(/^by\s+/i, "") || "Bandcamp",
    ...
    function collectionTrackFromCard(card) {
      ...
      const itemData = parseJsonAttribute(metadataNode, "data-item-json") || {};
      const itemLink = card.querySelector(".collection-item-gallery-container a.item-link[href]") || card.querySelector("a.item-link[href*='.bandcamp.com/album/'], a.item-link[href*='.bandcamp.com/track/']");
      const pageUrl = resolvedTrackPageUrl({ pageUrl: itemData.item_url || itemLink?.href });
      const releaseTitle = card.dataset.title || itemData.item_title || elementText(card, [".collection-item-gallery-container .collection-item-title", ".collection-item-title"]);
      const title = itemData.featured_track_title || card.dataset.tracktitle || elementText(card, [".fav-track-title", ".featured-track .title", ".track-title"]) || releaseTitle;
      if (!pageUrl || !title) return null;
      const artist = String(itemData.band_name || elementText(card, [".collection-item-gallery-container .collection-item-artist", ".collection-item-artist"])).replace(/^by\s+/i, "") || "Bandcamp";
    ...
    function runNativeCollectionControl(control) {
      if (!control) return;
      releaseExplicitPlaybackClear();
      collectionNativeFallbackUntil = Date.now() + 5e3;
    ...
    function injectPlaylistButtons({ incremental = false } = {}) {
      ...
      if (onFeedPage) {
        ...
        for (const purchaseAction of feedControls) {
          ...
          const wishlistItem = actionList?.querySelector("li[id^='collect-item_']") || [...actionList?.querySelectorAll("li") || []].find((item) => /^(?:in )?wishlist$/i.test(pageActionControlText(item)));
          ...
          const wishlistControl = wishlistItem?.querySelector(".wishlist-msg") || [...wishlistItem?.querySelectorAll("a, button") || []].find((control) => /^wishlist$/i.test(pageActionControlText(control)));
          const wishlistedControl = wishlistItem?.querySelector(".wishlisted-msg > span:first-child") || [...wishlistItem?.querySelectorAll("a, button") || []].find((control) => /^in wishlist$/i.test(pageActionControlText(control)));
          if (wishlistControl) {
      ...
      if (document.querySelector("#DiscoverApp, .results-grid")) {
        ...
        const pendingDiscoverPlayControls = incremental ? allDiscoverPlayControls.filter((control) => !control.closest(".results-grid-item") || control.dataset.bandkitDiscoverActionScanned !== "true") : allDiscoverPlayControls;
        ...
        for (const playControl of visibleDiscoverPlayControls) {
          ...
          const parent = resultCard ? playControl.parentElement : playControl.closest(".focused-result, .discover-detail") || playControl.parentElement;
          if (!parent) continue;
      ...
      for (const node of [...new Set(candidates)]) {
        ...
        try {
          fallbackIsTrack = new URL(fallbackPageUrl).pathname.includes("/track/");
        } catch {
        }
        ...
        const track = available.find((item) => nodeStream && item.url === nodeStream) || available.find((item) => String(item.title || "").replace(/\s+/g, " ").trim().toLowerCase() === normalizedTitle) || feedTrack || (fallbackIsTrack ? { ...fallback, id: String(node.dataset.trackid || node.dataset.trackId || title), duration: 0, url: "" } : null);
        ...
        let buyTrack = isClassicTrackRow ? [...node.querySelectorAll("a, button")].find((control) => /^buy track$/i.test(pageActionControlText(control))) : null;
        if (isClassicTrackRow) {
          const actionCell = node.querySelector(".download-col, .track-row-actions");
          if (!actionCell) continue;
          let button2 = node.querySelector(".bandcamp-hub-page-playlist.is-track-action");
          if (!button2) {
            button2 = createPagePlaylistButton("a");
            button2.className = `${buyTrack?.className || ""} bandcamp-hub-page-playlist is-track-action`.trim();
            if (buyTrack) buyTrack.before(button2);
            else actionCell.append(button2);
          }
          if (!buyTrack) {
            ...
            button2.after(buyTrack);
          }
          button2.classList.remove("bandcamp-hub-page-buy");
          button2.style.removeProperty("--hub-buy-icon");
    ...
    function elementText(root2, selectors) {
      for (const selector of selectors) {
        const value = root2.querySelector(selector)?.textContent?.replace(/\s+/g, " ").trim();
    ...
    function queueCartArtistResolution(items) {
      const unresolved = [...new Map((items || []).filter((item) => isGenericCartArtist(item.artist) && safeBandcampUrl(item.url) && !cartArtistPending.has(item.url) && !cartArtistAttempted.has(item.url)).map((item) => [item.url, { url: item.url }])).values()];
    ...
    function scanLiveCart() {
      ...
      const parsedFromBridge = Array.isArray(bridgedCart) ? bridgedCart.map((item) => {
        ...
        const payloadArtist = item.artist_name || item.band_name || item.band_title || item.selling_band_name || item.artist || item.artist_title || firstReleaseValue(item, ["artist_name", "band_name", "band_title", "selling_band_name", "artist", "artist_title"]);
      ...
      const candidates = [...document.querySelectorAll("#sidecart #item_list > *, [data-test='cart-item'], .cart-item")].filter((node) => node.children.length > 0 && getComputedStyle(node).display !== "none");
      ...
      const signature = (items) => JSON.stringify(items.map(({ title, artist, price, currency, url, art, restore }) => ({
        ...
        url,
        art,
        restoreKey: restore ? `${restore.item_type}:${restore.item_id}:${restore.option_id ?? ""}` : ""
      })));
    ...
    function scheduleLivePlayerMaintenance(delay = 7e3) {
    ...
    function scanLivePlayer() {
      ...
      for (const candidate of document.querySelectorAll("audio")) {
        ...
        candidate.addEventListener("play", async () => {
          ...
          const sameSeamlessFeedTrack = Boolean(feedState?.track && seamless.enabled && matchingQueueTrack([seamless.track], feedState.track));
    ...
    function recordListeningActivity() {
      ...
      state.activity.unshift({
        ...
        title: live.title,
        artist: live.artist,
        time: "Just now \xB7 recorded locally",
    ...
    launcher.addEventListener("pointerdown", (event) => {
      ...
      const finish = (finishEvent) => {
        if (!launcherDrag || finishEvent?.pointerId !== void 0 && finishEvent.pointerId !== launcherDrag.pointerId) return;
    ...
    document.addEventListener("click", (event) => {
      ...
      if (feedControl) {
        ...
        const controlsCurrentSeamlessTrack = Boolean(
          seamless.enabled && clickedTrackId && clickedTrackId === String(seamless.track?.id || "")
        );
        if (controlsCurrentSeamlessTrack) {
          event.preventDefault();
          event.stopImmediatePropagation();
      ...
      if (discoverControl) {
        ...
        const controlsCurrentSeamlessTrack = Boolean(
          discoverControl.closest(".discover-player") && seamless.enabled && matchingQueueTrack([seamless.track], discover?.track)
        );
        if (controlsCurrentSeamlessTrack) {
          event.preventDefault();
          event.stopImmediatePropagation();
    ...
    function scrubTarget() {
      ...
      return {
        duration,
        currentTime: Number(scrubSlider.value) / 1e3 * duration
      };
    }
    ...
    const runtimeMessageHandler = (message, _sender, sendResponse) => {
      ...
      if (message?.type === MESSAGES.RESTORE_CART) {
        restoreSavedCart(message.items).then(sendResponse).catch((error) => sendResponse({ ok: false, error: error.message || "Cart restoration failed." }));
        return true;
      }
    ...
    chrome.storage.onChanged?.addListener((changes, area) => {
      ...
      const musicBarCustomWidth = Math.round(Math.max(480, Math.min(2e3, Number(incoming.musicBarCustomWidth) || 900)));
      const appearance = {
        ...state.appearance,
        ...incoming.appearance || {},
        savedThemes: Array.isArray(incoming.appearance?.savedThemes) ? incoming.appearance.savedThemes.filter((theme) => theme?.id && theme?.label && theme?.accent && theme?.surface).slice(0, 12) : state.appearance.savedThemes
      };
      const appearanceChanged = JSON.stringify(appearance) !== JSON.stringify(state.appearance);
      if (JSON.stringify(savedCarts) === JSON.stringify(state.savedCarts) && JSON.stringify(playlist) === JSON.stringify(state.playlist) && playlistMode === state.playlistMode && JSON.stringify(savedPlaylists) === JSON.stringify(state.savedPlaylists) && autoAnalyzeTracks === (state.autoAnalyzeTracks !== false) && showTrackKeys === (state.showTrackKeys !== false) && pageActionLabels === Boolean(state.pageActionLabels) && recordPlaylistMetadata === (state.recordPlaylistMetadata !== false) && scrubberStyle === state.scrubberStyle && musicBarSize === state.musicBarSize && musicBarWidth === state.musicBarWidth && musicBarCustomWidth === state.musicBarCustomWidth && !appearanceChanged) return;
    ...
    async function init() {
      ...
      const earlyModernPageBootstrap = document.documentElement.dataset.bandkitModernBootstrap === "true" ? globalThis.BandKitModernPagesBootstrap : null;
      ...
      const modernCssPromise = earlyModernPageBootstrap ? Promise.resolve("") : fetch(modernStyleUrl.href).then((response) => response.text());
      ...
      function syncNativeHeaderCart(nav) {
        ...
        const iconNativeCarts = roots.flatMap((headerRoot) => [...headerRoot.querySelectorAll('use[href$="#menubar-cart-icon"], use[xlink\\:href$="#menubar-cart-icon"]')].map((icon) => icon.closest("a, button, [role='button'], li")).filter(Boolean));
        const nativeCarts = [.../* @__PURE__ */ new Set([...labelledNativeCarts, ...iconNativeCarts])].filter((control) => !root.contains(control));
        const hideHeaderCart = Boolean(state.appearance.hideHeaderCart);
      ...
      function syncExtensionStackingAncestor() {
        ...
        const nextAncestor = root.closest("header, #menubar-wrapper") || shadowHost?.closest?.("menu-bar, header, #menubar-wrapper") || null;
      ...
      function mountLauncherInHeader({ allowFloating = false } = {}) {
        ...
        if (modernNav && feedItem?.parentElement === modernNav) {
          ...
          const nativeColor = nativeIconStyle?.fill && nativeIconStyle.fill !== "none" ? nativeIconStyle.fill : nativeIconStyle?.stroke || getComputedStyle(feedControl).color;
      ...
      const observedHeaderMountTargets = /* @__PURE__ */ new WeakSet();
      ...
      if (savedState) {
        state = app.replaceState({
          ...
          appearance: {
            ...state.appearance,
            ...savedState.appearance || {},
            savedThemes: Array.isArray(savedState.appearance?.savedThemes) ? savedState.appearance.savedThemes.filter((theme) => theme?.id && theme?.label && theme?.accent && theme?.surface).slice(0, 12) : []
          },
          dj: {
            ...state.dj,
            ...savedState.dj || {}
      ...
      state.musicBarCustomWidth = Math.round(Math.max(480, Math.min(2e3, Number(state.musicBarCustomWidth) || 900)));
      ...
      const savedAtDate = state.cartSavedAt ? new Date(state.cartSavedAt) : /* @__PURE__ */ new Date();
      const initialAutoSave = cartAutosave.upsertAutoSavedCart(state.savedCarts, state.cart, {
        savedAt: Number.isNaN(savedAtDate.getTime()) ? (/* @__PURE__ */ new Date()).toISOString() : savedAtDate.toISOString(),
        sourcePage: portableBandcampUrl(location.href),
        summary: state.cartSummary
      });
      ...
      state.wishlistTrackKeys = Array.isArray(state.wishlistTrackKeys) ? state.wishlistTrackKeys.filter((key) => typeof key === "string" && key).slice(-500) : [];
      ...
      if (!scanTimer && !scanIdleCallback) scheduleLivePlayerMaintenance();
      seamlessSyncTimer = window.setInterval(() => void syncSeamlessState(), 3e3);
