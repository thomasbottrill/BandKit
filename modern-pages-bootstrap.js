(() => {
  ...
  const finish = () => {
    ...
    root.dataset.bandkitModernReady = "true";
  };
  ...
  chrome.storage.local.get("bandcampHubState", (stored) => {
    if (chrome.runtime.lastError) {
      restoreClassicPage();
      return;
    }
    const enabled = stored?.bandcampHubState?.appearance?.modernReleasePages !== false;
