(() => {
  ...
  (() => {
    ...
    function isAutoSavedCart(snapshot) {
      ...
      return Boolean(snapshot?.autoSaved) || snapshot?.id === AUTO_SAVED_CART_ID || normalizedName === "auto saved cart" || normalizedName === "autosaved cart";
    }
    ...
    function normalizeSavedCarts(savedCarts) {
      const candidates = (Array.isArray(savedCarts) ? savedCarts : []).filter((snapshot) => snapshot && Array.isArray(snapshot.items) && (snapshot.items.length || !isAutoSavedCart(snapshot)));
    ...
    function upsertAutoSavedCart(savedCarts, items, options = {}) {
      ...
      const matchingNamedCart = next.some((snapshot2) => !isAutoSavedCart(snapshot2) && cartSignature(snapshot2.items) === signature);
      if (matchingNamedCart) {
        next = next.filter((snapshot2) => !isAutoSavedCart(snapshot2));
      ...
      if (existing && cartSignature(existing.items) === signature && summarySignature(existing.summary) === summarySignature(options.summary)) {
        return { savedCarts: next, changed: !sameSavedCarts(original, next) };
      }
      const savedAt = options.savedAt || (/* @__PURE__ */ new Date()).toISOString();
      const snapshot = {
        id: AUTO_SAVED_CART_ID,
        name: "Auto-saved cart",
        autoSaved: true,
    ...
    function saveNamedCart(savedCarts, items, name, options = {}) {
      ...
      const signature = cartSignature(cartItems);
      const autoIndex = next.findIndex((snapshot2) => isAutoSavedCart(snapshot2) && cartSignature(snapshot2.items) === signature);
