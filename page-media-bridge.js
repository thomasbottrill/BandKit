(() => {
  ...
  (() => {
    ...
    function releaseData(releases) {
      const source = Array.isArray(releases) ? releases : releases && typeof releases === "object" ? Object.values(releases) : [];
    ...
    function waitForCartItem(key, previousLocalIds, timeout = 1e4) {
    ...
    function waitForCartItemRemoval(item, timeout = 1e4) {
      return new Promise((resolve) => {
        const startedAt = Date.now();
        const check = () => {
          const stillPresent = window.Sidecart?.cart_items?.some((candidate) => item.local_id != null ? candidate.local_id === item.local_id : cartItemKey(candidate) === cartItemKey(item));
    ...
    async function removeCartItem(request) {
      ...
      if (!window.Sidecart?.delete_from_cart || !Array.isArray(window.Sidecart.cart_items)) {
        ...
        const match = requestedItem && window.Sidecart.cart_items.find((candidate) => requestedItem.local_id != null && candidate.local_id === requestedItem.local_id) || requestedItem && window.Sidecart.cart_items.find((candidate) => cartItemKey(candidate) === cartItemKey(requestedItem));
        if (!match) {
          result.removed = true;
