...
import { registerNowPlayingActions, setupNowPlayingActions } from "./runtime/now-playing-actions.js";
import { registerDjPlayback, setupDjPlayback } from "./runtime/dj-playback.js";
import { registerDjPageTools, setupDjPageTools } from "./runtime/dj-page-tools.js";
import { registerCollectionViews, setupCollectionViews } from "./runtime/collection-views.js";
import { registerCollectionPlaylistIntegration, setupCollectionPlaylistIntegration } from "./runtime/collection-playlist-integration.js";
...
(async () => {
  ...
  for (const install of registerNowPlayingActions) install(r);
  for (const install of registerDjPlayback) install(r);
  for (const install of registerDjPageTools) install(r);
  for (const install of registerCollectionViews) install(r);
  for (const install of registerCollectionPlaylistIntegration) install(r);
  ...
  for (const install of setupNowPlayingActions) install(r);
  for (const install of setupDjPlayback) install(r);
  for (const install of setupDjPageTools) install(r);
  for (const install of setupCollectionViews) install(r);
  for (const install of setupCollectionPlaylistIntegration) install(r);
